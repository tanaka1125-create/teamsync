...
- 候補ごとの任意コメント入力
- 一部候補だけの新規回答
- 回答一覧の回答者名をクリックして既存回答を編集
- 回答者ごとの削除ボタンと2段階の削除確認
- 新規回答時の同名登録を拒否し、名前入力による自動更新を廃止
- 回答者・回答データを1回の処理で保存するデータベース関数
- 候補日時ごとの○・△・×・未回答人数の集計
- 参加者ごとの回答・コメント一覧表
- 回答表の文字サイズと行間を読みやすく調整し、見出し行をさらに強調
...
新規の出欠回答は `submit_event_responses` 関数を使い、名前と選択した候補の○・△・×、任意コメントを保存します。同じイベント内ですでに使われている名前は新規登録できません。

既存回答は、回答一覧の回答者名から `response.html?id=<イベントID>&participant=<回答者ID>` を開き、`update_event_responses` 関数で変更します。イベント専用URLを知っている人は回答一覧の閲覧・新規回答・既存回答の編集ができます。

回答の削除は `delete_event_participant` 関数を使い、回答一覧で選択した回答者と、その回答者に紐づく○・△・×・コメントを削除します。誤操作を防ぐため、削除ボタンは6秒以内に2回押した場合だけ実行します。
